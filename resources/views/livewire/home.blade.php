    ...
    <div class="flex">
        ...
        <div class="min-w-[455px] py-2 px-4 overflow-y-scroll max-h-[80vh]">
            @if (count($generatedLabels) > 0)
                <div class="w-[380px] space-y-3 text-gray-900">
                    @foreach ($generatedLabels as $label => $weight)
                        <div x-on:click="selectedLabels.includes({{ $label }}) ? selectedLabels = selectedLabels.filter(id => id !== {{ $label }}) : selectedLabels.push({{ $label }}); console.log(selectedLabels)"
                            class="flex items-center gap-x-4 justify-between cursor-pointer relative bg-white">
                            ...
                            <div class="h-[250px] bg-white flex flex-col justify-start items-center hover:ring-2 hover:ring-indigo-400">
                                ...
                                <div class="flex w-full h-full">
                                    <div class="border-r-1 w-full flex flex-col">
                                        <div class="flex flex-col items-center justify-center border-b-1 pb-3">
                                            ...
                                            <span>{!! DNS1D::getBarcodeSVG((int)$requestNumber, 'C128', 2, 40) !!}</span>
                                        </div>
                                        <div class="flex flex-col items-center justify-center">
                                            <span class="text-[16px] font-bold">NUMPAL</span>
                                            <span>{!! DNS1D::getBarcodeSVG((int)$paletNumber, 'C128', 2, 40) !!}</span>
                                        </div>
                                    </div>
                                    <div class="w-[200px] flex items-center justify-center">
                                        {!! DNS2D::getBarcodeHTML('(91)'.str_pad($codpro, 13, '0', STR_PAD_LEFT).'(10)'.str_pad($paletNumber, 9, '0', STR_PAD_LEFT).'-'.($loop->index + 1).'(15)'.$datfvi.'(37)'.str_pad($weight, 4, '0', STR_PAD_LEFT), 'QRCODE', 3, 3) !!}
                                    </div>
                                </div>
                            </div>
                        </div>
